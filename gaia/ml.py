...
class SpatialModel:
    ...
    def __init__(self, estimator, n_neighbors=20):
        ...
        if is_classifier(estimator) or is_regressor(estimator):
            ...
            raise TypeError("It does not seem to be a scikit-learn estimator "
                            "for supervised learning.")
        
        if n_neighbors>2 and isinstance(n_neighbors, int):
            self.n_neighbors = n_neighbors
        else:
            raise ValueError("Number of neighbors (%s) is invalid:"
    ...
    def predict(self, X, Z):
        ...
        for line in range(X.shape[0]):
            ...
            if not self.sample_weight:
                ...
                local_weights = None
            self.models.append(self.estimator.fit(local_train_X, local_train_y, local_weights).copy())
